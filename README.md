...
type Config struct {
	GoogleClientID     string `env:"GOOGLE_CLIENT_ID,required,notEmpty" secret:"true"`
	GoogleClientSecret string `env:"GOOGLE_CLIENT_SECRET,required,notEmpty" secret:"true"`
	GoogleRefreshToken string `env:"GOOGLE_REFRESH_TOKEN,required,notEmpty" secret:"true"`
...
fmt.Println("The Google Client ID is: %s", cfg.GoogleClientID)
```
...
services:
  worker:
    ...
    environment:
      LOG_LEVEL: info
      # GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, and GOOGLE_REFRESH_TOKEN are
      # secrets; omit them here — ultra resolves and injects them at run time.
```
...
The CLI discovers every app under `apps/<app>/config`, resolves its secrets from the provider, forwards them into that app's container.
